...
use BlackCMS\Language\Models\Language;
...
class LanguageManager
{
    ...
    public function getSupportedLocales()
    {
        ...
        foreach ($languages as $language) {
            if (!in_array(
                $language->lang_id,
                json_decode(setting("language_hide_languages", "[]"), true)
            )
            ) {
    ...
    public function getActiveLanguage($select = ["*"])
    {
        ...
        $this->activeLanguages = $this->languageRepository->getActiveLanguage(
            $select
        );
    ...
    public function getRelatedLanguageItem($id, $uniqueKey)
    {
         ...
         * @var Builder $meta
         */
        $meta = $this->languageMetaRepository
            ->getModel()
            ->where("lang_meta_origin", $uniqueKey);
    ...
    public function getLocalizedURL(
        ...
        if ($locale &&
            ($translatedRoute = $this->findTranslatedRouteByUrl(
                $url,
                $attributes,
                $this->currentLocale
            ))
        ...
        if ($translatedRoute = $this->findTranslatedRouteByPath(
            $parsedUrl["path"],
            $urlLocale
        )
        ) {
        ...
        if (!empty($locale)) {
            if ($forceDefaultLocation ||
                ...
                $parsedUrl["path"] =
                    $locale . "/" . ltrim($parsedUrl["path"], "/");
            }
        }
        $parsedUrl["path"] = ltrim(
            ltrim($basePath, "/") . "/" . $parsedUrl["path"],
            "/"
        );
    ...
    protected function extractAttributes($url = false, $locale = "")
    {
        if (!empty($url)) {
            ...
            $attributes = $this->normalizeAttributes(
                $this->router->current()->parameters()
            );
    ...
    public function getURLFromRouteNameTranslated(
        ...
        if (is_string($locale) &&
            $this->translator->has($transKeyName, $locale)
        ) {
            $translation = $this->translator->get($transKeyName, [], $locale);
            $route .= "/" . $translation;

            $route = $this->substituteAttributesInRoute(
                $attributes,
                $route,
                $locale
            );
    ...
    public function hideDefaultLocaleInURL()
    {
        return setting(
            ...
            config("addons.language.general.hideDefaultLocaleInURL")
        );
    }
     ...
     */
    protected function substituteAttributesInRoute(
        $attributes,
        $route,
        $locale = null
    ) {
    ...
    protected function findTranslatedRouteByUrl($url, $attributes, $locale)
    {
        ...
        foreach ($this->translatedRoutes as $translatedRoute) {
            $routeName = $this->getURLFromRouteNameTranslated(
                $locale,
                $translatedRoute,
                $attributes
            );

            if ($this->getNonLocalizedURL($routeName) ==
                $this->getNonLocalizedURL($url)
            ) {
    ...
    public function getLocalesMapping()
    {
        if (empty($this->localesMapping)) {
            $this->localesMapping = config(
                "addons.language.general.localesMapping"
            );
        }
    ...
    protected function findTranslatedRouteByPath($path, $urlLocale)
    {
        // Check if this url is a translated url
        foreach ($this->translatedRoutes as $translatedRoute) {
            if ($this->translator->get($translatedRoute, [], $urlLocale) ==
                rawurldecode($path)
            ) {
                return $translatedRoute;
            }
    ...
    public function getCurrentLocaleName()
    {
        ...
        return Arr::get(
            ...
            $this->getCurrentLocale() . ".lang_name"
        );
    }
    ...
    public function getCurrentLocaleRTL()
    {
        ...
        return Arr::get(
            $this->supportedLocales,
            $this->getCurrentLocale() . ".lang_is_rtl"
        );
    }
    ...
    public function getCurrentLocaleCode()
    {
        ...
        return Arr::get(
            $this->supportedLocales,
            $this->getCurrentLocale() . ".lang_code"
        );
    }
    ...
    {
        $language = collect($this->supportedLocales)
            ->where("lang_code", $localeCode)
            ->first();
    ...
    public function getDefaultLocaleCode()
    {
        ...
        return Arr::get(
            $this->supportedLocales,
            $this->getDefaultLocale() . ".lang_code",
            config("app.locale")
        );
    }
    ...
    public function getCurrentLocaleFlag()
    {
        ...
        return Arr::get(
            $this->supportedLocales,
            $this->getCurrentLocale() . ".lang_flag"
        );
    }
    ...
    public function getRouteNameFromAPath($path)
    {
        ...
        foreach ($this->translatedRoutes as $route) {
            if ($this->substituteAttributesInRoute(
                $attributes,
                $this->translator->get($route)
            ) === $path
            ) {
                return $route;
            }
    ...
    public function saveLanguage($screen, $request, $data)
    {
        ...
        if (!empty($defaultLanguage)) {
            if ($data != false &&
                in_array(get_class($data), $this->supportedModels())
            ) {
    ...
    public function getDefaultLanguage($select = ["*"])
    {
        ...
        $this->defaultLanguage = $this->languageRepository->getDefaultLanguage(
            $select
        );
        $this->defaultLanguageSelect = $select;
    ...
    public function supportedModels()
    {
        return apply_filters(
            ...
            config("addons.language.general.supported", [])
        );
    }
    ...
    public function registerModule($model)
    {
        ...
        config([
            "addons.language.general.supported" => array_merge(
                config("addons.language.general.supported", []),
                $model
            ),
        ]);
    ...
    public function setLocale($locale = null)
    {
        if (empty($locale) || !is_string($locale)) {
            ...
            if ($localeFromRequest &&
                array_key_exists($localeFromRequest, $this->supportedLocales)
            ) {
    ...
    public function getSwitcherUrl(string $localeCode, string $languageCode)
    {
        if (count($this->switcherURLs)) {
            $url = collect($this->switcherURLs)
                ->where("lang_code", $languageCode)
                ->first();

            if ($url) {
                return rtrim($url["url"], "/") == rtrim(url(""), "/")
                    ? url($localeCode)
                    : $url["url"];
            }
        }
        ...
        return $showRelated
            ? $this->getLocalizedURL($localeCode)
            : url($localeCode);
    }
