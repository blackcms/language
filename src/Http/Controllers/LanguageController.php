...
namespace BlackCMS\Language\Http\Controllers;
...
use BlackCMS\Language\Http\Requests\LanguageRequest;
...
class LanguageController extends BaseController
{
    ...
    {
        page_title()->setTitle(trans("addons/language::language.name"));

        Assets::addScriptsDirectly([
            "vendor/core/addons/language/js/language.js",
        ]);
        ...
        return view(
            "addons/language::index",
            compact("languages", "flags", "activeLanguages")
        );
    }
        ...
        try {
            ...
            if ($language) {
                return $response
                    ->setError()
                    ->setMessage(
                        trans("addons/language::language.added_already")
                    );
            }
            ...
            if (!File::isWritable(resource_path("lang")) ||
                !File::isWritable(resource_path("lang/vendor"))
            ) {
                return $response
                    ->setError(true)
                    ->setMessage(
                        trans(
                            "addons/translation::translation.folder_is_not_writeable"
                        )
                    );
            }
            ...
            if (!File::isDirectory(resource_path("lang/" . $locale))) {
                ...
                $this->createLocaleInPath(
                    resource_path("lang/vendor/core"),
                    $locale
                );
                $this->createLocaleInPath(
                    resource_path("lang/vendor/packages"),
                    $locale
                );
                $this->createLocaleInPath(
                    resource_path("lang/vendor/addons"),
                    $locale
                );

                $themeLocale = Arr::first(
                    scan_folder(theme_path(Theme::getThemeName() . "/lang"))
                );

                if ($themeLocale) {
                    File::copy(
                        theme_path(
                            Theme::getThemeName() . "/lang/" . $themeLocale
                        ),
            ...
            $language = $this->languageRepository->createOrUpdate(
            ...
            try {
                    ...
                    foreach ($models as $model) {
                        ...
                        foreach ($referenceIds as $referenceId) {
                            $data[] = [
                                ...
                                "lang_meta_origin" => md5(
                                    $referenceId . $model . time()
                                ),
                            ];
                        }
                ...
                return $response
                    ->setData(
                        view("addons/language::partials.language-item", [
                            "item" => $language,
                        ])->render()
                    )
     ...
     * @return BaseHttpResponse
     */
    public function postChangeItemLanguage(
        Request $request,
        BaseHttpResponse $response
    ) {
        $referenceId =
            $request->input("reference_id") ?:
            $request->input("lang_meta_created_from");
        ...
        if ($currentLanguage) {
            $others = $others
                ->where(
                    "lang_meta_code",
                    "!=",
                    $request->input("lang_meta_current_language")
                )
        ...
        foreach ($languages as $language) {
            if (!array_key_exists($language->lang_code, $data) &&
                $language->lang_code !=
                    $request->input("lang_meta_current_language")
            ) {
                $data[$language->lang_code]["lang_flag"] = $language->lang_flag;
    ...
    public function getSetDefault(Request $request, BaseHttpResponse $response)
    {
        ...
        try {
            if ($defaultLanguage->lang_id != $request->input("lang_id")) {
                ...
                foreach ($widgets as $widget) {
                    $widget->theme =
                        $widget->theme . "-" . $defaultLanguage->lang_code;
                    $widget->save();
                }
                ...
                $themeOptions = Setting::where(
                    "key",
                    "NOT LIKE",
                    "theme-" . $themeName . "-" . $language->lang_code . "-%"
                )->get();
        ...
        event(
            new UpdatedContentEvent(LANGUAGE_MODULE_NAME, $request, $language)
        );
    ...
    public function postEditSettings(
        ...
        $settingStore
            ->set(
                ...
                $request->input("language_hide_default", false)
            )
            ->set("language_display", $request->input("language_display"))
            ->set(
                "language_switcher_display",
                $request->input("language_switcher_display")
            )
            ->set(
                "language_hide_languages",
                json_encode($request->input("language_hide_languages", []))
            )
            ->set(
                "language_auto_detect_user_language",
                $request->input("language_auto_detect_user_language")
            )
            ->set(
