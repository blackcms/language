    ...
    public function handle(RenderingSingleEvent $event)
    {
        try {
            if (defined("THEME_FRONT_HEADER")) {
                add_filter(
                    THEME_FRONT_HEADER,
                    function ($header) use ($event) {
                        $urls = [];

                        if (in_array(
                            $event->slug->reference_type,
                            Language::supportedModels()
                        )
                        ) {
                            if ($event->slug->reference_type == Page::class &&
                                BaseHelper::isHomepage(
                                    $event->slug->reference_id
                                )
                            ) {
                                ...
                                foreach ($slug as $item) {
                                    if (!empty(
                                        $languageMeta[$item->reference_id]
                                    )
                                    ) {
                                        $locale = Language::getLocaleByLocaleCode(
                                            $languageMeta[$item->reference_id]
                                        );
                        ...
                        return $header .
                            view(
                                "addons/language::partials.hreflang",
                                compact("urls")
                            )->render();
                    },
                    55
