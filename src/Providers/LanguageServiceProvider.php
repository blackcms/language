...
use BlackCMS\Language\Facades\LanguageFacade;
use BlackCMS\Language\Http\Middleware\LocaleSessionRedirect;
use BlackCMS\Language\Http\Middleware\LocalizationRedirectFilter;
use BlackCMS\Language\Http\Middleware\LocalizationRoutes;
use BlackCMS\Language\Models\Language as LanguageModel;
use BlackCMS\Language\Models\LanguageMeta;
use BlackCMS\Language\Repositories\Caches\LanguageCacheDecorator;
use BlackCMS\Language\Repositories\Caches\LanguageMetaCacheDecorator;
use BlackCMS\Language\Repositories\Eloquent\LanguageMetaRepository;
use BlackCMS\Language\Repositories\Eloquent\LanguageRepository;
use BlackCMS\Language\Repositories\Interfaces\LanguageInterface;
use BlackCMS\Language\Repositories\Interfaces\LanguageMetaInterface;
...
class LanguageServiceProvider extends ServiceProvider
{
    ...
    {
        $this->setNamespace("addons/language")->loadAndPublishConfigurations([
            "general",
        ]);

        $this->app->bind(LanguageInterface::class, function () {
            return new LanguageCacheDecorator(
                new LanguageRepository(new LanguageModel())
            );
        });

        $this->app->bind(LanguageMetaInterface::class, function () {
            return new LanguageMetaCacheDecorator(
                new LanguageMetaRepository(new LanguageMeta())
            );
        });
        ...
        $router->aliasMiddleware(
            ...
            LocalizationRedirectFilter::class
        );
        $router->aliasMiddleware(
    ...
    {
        $this->setNamespace("addons/language")
            ->loadHelpers()
        ...
        if (!is_in_admin()) {
            add_filter(
                ...
                [$this, "addLanguageMiddlewareToPublicRoute"],
                958
            );
        }
        ...
        Event::listen(RouteMatched::class, function () {
            dashboard_menu()->registerItem([
                "id" => "cms-addons-language",
                "priority" => 2,
                "parent_id" => "cms-core-settings",
                "name" => "addons/language::language.name",
                "icon" => null,
            ...
            Assets::addScriptsDirectly(
                "vendor/core/addons/language/js/language-global.js"
            )->addStylesDirectly([
                "vendor/core/addons/language/css/language.css",
            ]);
        });

        $this->app->booted(function () {
            if (defined("THEME_OPTIONS_MODULE_NAME")) {
                Language::registerModule(THEME_OPTIONS_MODULE_NAME);
            }

            if (defined("WIDGET_MANAGER_MODULE_NAME")) {
                Language::registerModule(WIDGET_MANAGER_MODULE_NAME);
            }
        });
        ...
        if (!empty($defaultLanguage)) {
            ...
            add_action(
                ...
                [$this, "addCurrentLanguageEditingAlert"],
                55,
                2
            );
            add_action(
                BASE_ACTION_BEFORE_EDIT_CONTENT,
                [$this, "getCurrentAdminLanguage"],
                55,
                2
            );
            if (defined("THEME_OPTIONS_MODULE_NAME")) {
                    ...
                    $this->app->booted(function () {
                        Theme::asset()
                            ...
                            ->add(
                                "language-css",
                                asset(
                                    "vendor/core/addons/language/css/language-public.css"
                                ),
                                [],
                                [],
                                "1.0.0"
                        ...
                        Theme::asset()
                            ...
                            ->add(
                                "language-public-js",
                                asset(
                                    "vendor/core/addons/language/js/language-public.js"
                                ),
                                ["jquery"],
                                [],
                                "1.0.0"
            ...
            add_filter(
                ...
                [$this, "checkItemLanguageBeforeShow"],
                50,
                2
            );
            if (setting(
                ...
                add_filter(
                    ...
                    [$this, "getRelatedDataForOtherLanguage"],
                    50,
                    2
                );
            }
            ...
            add_filter(
                ...
                [$this, "addLanguageSwitcherToTable"],
                247,
                2
            );
            add_filter(
                BASE_FILTER_TABLE_QUERY,
                [$this, "getDataByCurrentLanguage"],
                157
            );
            add_filter(
                BASE_FILTER_BEFORE_GET_ADMIN_LIST_ITEM,
                [$this, "checkItemLanguageBeforeGetAdminListItem"],
                50
            );
    ...
    {
        if (!empty($object) &&
            in_array(get_class($object), Language::supportedModels())
        ) {
            MetaBox::addMetaBox(
                "language_wrap",
                trans("addons/language::language.name"),
    ...
    public function addLanguageMetaBoxForThemeOptionsAndWidgets($data, $screen)
    {
        $route = null;
        switch ($screen) {
            case THEME_OPTIONS_MODULE_NAME:
                $route = "theme.options";
                break;
            case WIDGET_MANAGER_MODULE_NAME:
                $route = "widgets.index";
                break;
        }
    ...
    public function setSlugPrefix(string $prefix)
    {
        ...
        if ($currentLocale &&
            (!setting("language_hide_default") ||
                $currentLocale != Language::getDefaultLocale())
        ) {
            if (!$prefix) {
    ...
    public function languageMetaField()
    {
        ...
        if ($args[0] && $args[0]->id) {
            $meta = $this->app->make(LanguageMetaInterface::class)->getFirstBy(
                ...
                ["lang_meta_code", "reference_id", "lang_meta_origin"]
            );
        ...
        if ($meta) {
            $related = Language::getRelatedLanguageItem(
                $meta->reference_id,
                $meta->lang_meta_origin
            );
        }
    ...
    public function addCurrentLanguageEditingAlert($request, $data = null)
    {
        ...
        if ($data && in_array($model, Language::supportedModels())) {
            ...
            if (!empty($code)) {
                ...
                $language = $this->app
                    ->make(LanguageInterface::class)
                    ->getFirstBy(["lang_code" => $code], ["lang_name"]);
                if (!empty($language)) {
            ...
            echo view(
                "addons/language::partials.notification",
                compact("language")
            )->render();
        }
    ...
    public function addLanguageTableHeading($headings, $model)
    {
        if (in_array(get_class($model), Language::supportedModels())) {
            if (is_in_admin() &&
                Auth::check() &&
                !Auth::user()->hasAnyPermission($this->getRoutes())
            ) {
                return $headings;
            }
    ...
    public function addLanguageColumn($data, $model)
    {
        if ($model &&
            ...
            if (is_in_admin() &&
                Auth::check() &&
                !Auth::user()->hasAnyPermission($route)
            ) {
                return $data;
            }

            return $data->addColumn("language", function ($item) use (
                $model,
                $route
            ) {
                $relatedLanguages = [];
                ...
                if (Language::getCurrentAdminLocaleCode() === "all") {
                    $currentLanguage = $this->app
                        ->make(LanguageMetaInterface::class)
                        ->getFirstBy([
                ...
                foreach ($languages as $language) {
                    if ($language->lang_code == $currentLanguage) {
                        $data .= view(
                            "addons/language::partials.status.active",
                            compact("route", "item")
                        )->render();
                    } else {
                        $added = false;
                        if (!empty($relatedLanguages)) {
                            foreach ($relatedLanguages as $key => $relatedLanguage) {
                                if ($key == $language->lang_code) {
                                    $data .= view(
                                        "addons/language::partials.status.edit",
                        ...
                        if (!$added) {
                            $data .= view(
                                "addons/language::partials.status.plus",
                ...
                return view(
                    "addons/language::partials.language-column",
                    compact("data")
                )->render();
            });
        }
    ...
    {
        return view(
            "addons/language::partials.switcher",
            compact("options")
        )->render();
    }
    ...
    {
        return $this->getDataByCurrentLanguageCode(
            $data,
    ...
    public function addLanguageSwitcherToTable($buttons, $model)
    {
        if (in_array($model, Language::supportedModels())) {
            ...
            foreach ($activeLanguages as $item) {
                $languageButtons[] = [
                    "className" =>
                        ...
                        ($item->lang_code == $currentLanguage ? "active" : ""),
            ...
            $languageButtons[] = [
                "className" =>
                    "change-data-language-item " .
                    ("all" == $currentLanguage ? "active" : ""),
                "text" => Html::tag(
                    "span",
                    trans("addons/language::language.show_all"),
                    [
                        "data-href" => route(
                            "languages.change.data.language",
                            "all"
                ...
                "language" => [
                    ...
                    "text" =>
                        ...
                        Html::tag(
                            ...
                            " " .
                                trans(
                                    "addons/language::language.change_language"
                                ) .
                                " " .
                                Html::tag("span", "", [
                                    "class" => "caret",
